...
const UPDATE_CHANNEL_OPTIONS: Array<{ value: UpdateChannel; label: string; description: string }> = [
    {
        value: "stable",
        label: "Stable",
...
const AboutTab = ({
    ...
    const selectedChannel = UPDATE_CHANNEL_OPTIONS.find((option) => option.value === updateChannel);
            ...
            <div className="space-y-2">
                <div className="flex items-center justify-between gap-3">
                    ...
                    <div ref={channelMenuRef} className="relative">
                        ...
                        >
                            <span className="ui-color-primary">{selectedChannel?.label ?? "Stable"}</span>
                            <ChevronDown
                                size={12}
                                        ...
                                        >
                                            <div className="flex items-center justify-between gap-2">
                                                <div className="min-w-0">
                                                    <p className={`ui-text-body-sm-strong truncate ${
                                                        updateChannel === option.value ? "ui-color-cloud" : "ui-color-secondary"
                                                    }`}>
                                                        {option.label}
                                                    </p>
                                                    <p className="ui-text-meta ui-color-disabled truncate">{option.description}</p>
                                                </div>
                                                <span className={`shrink-0 ${updateChannel === option.value ? "ui-color-cloud" : "text-transparent"}`}>
