...
export function Dropdown<T extends string | number>({
  ...
  const renderBadges = (badges?: DropdownOption<T>["badges"], fixedBadgeSlots?: boolean) => {
        ...
        {badges.map((badge, index) =>
          badge.visible === false ? null : (
            <span
              key={`${badge.label}-${index}`}
              className={
                badge.highlighted ? "text-[var(--color-interactive)]" : "text-content-disabled"
              }
            >
              {badge.label}
            </span>
    ...
    <div className={`relative ${className}`} ref={containerRef}>
      ...
      >
        <div className="flex items-center gap-2 min-w-0">
          ...
          <span
            className={`truncate ${selectedOption ? "text-content-primary" : "text-content-muted"}`}
          >
            ...
            {searchable && (
              <div className="p-2 border-b border-border-secondary shrink-0">
                <div className="relative">
                  <Search
                    size={12}
                    className="absolute left-2.5 top-1/2 -translate-y-1/2 text-content-disabled"
                    aria-hidden="true"
                  />
                  <input
                    type="text"
