...
import type { LibraryFilter, LibraryItem, LibraryItemPatch, ModelStatus } from "../../../types";
...
const LibraryView = ({
  ...
  const { data: defaultModelKey = "" } = useSettings((settings) => settings.local_model, isActive);
  ...
  const selectedModel =
    installedModels.find((model) => model.key === defaultModelKey) ?? installedModels[0];
        ...
        <header className="flex flex-col gap-4 mb-4 mt-2 md:-mt-6">
          ...
          <div className="grid grid-cols-1 gap-3 md:grid-cols-[auto_minmax(14rem,1fr)_auto] md:items-center">
            ...
            <SegmentedControl
              ...
              onChange={(value) => setStatusFilter(value === "active" ? "transcribing" : value)}
              ariaLabel={t({
            ...
            <div className="w-full h-full min-h-0" onClick={(e) => e.stopPropagation()}>
              <LibraryModal
                ...
                onExport={(format, outputPath) =>
                  exportMutation.mutateAsync({ id: selectedItem.id, format, outputPath })
                }
                availableTags={availableTags}
              />
            </div>
