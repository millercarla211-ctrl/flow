...
const PENDING_RESTART_KEY = "flow_update_pending_restart";
...
export function UpdateChecker({ autoCheck = true }: UpdateCheckerProps) {
  ...
  const availableVersion = installed || !updateStatus?.available ? null : updateStatus.version;
  ...
  if (availableVersion) {
    return (
      <motion.div
        initial={{ opacity: 0, y: 4 }}
        animate={{ opacity: 1, y: 0 }}
        className="flex items-center gap-2 rounded-lg border border-border-secondary bg-surface-elevated px-3 py-2 h-[52px]"
      >
        <Download size={16} className="ui-color-primary shrink-0" />
        <div className="flex-1 min-w-0">
          <p className="ui-text-body-sm-strong ui-color-primary truncate">
            {t({
          ...
          {downloadError ? (
            <p className="ui-text-meta ui-color-error-subtle truncate" title={downloadError}>
              {downloadError}
            </p>
          ) : (
        ...
        <AnimatePresence mode="wait">
          {downloading ? (
            ...
            <motion.button
              ...
              className="rounded-lg bg-content-primary px-2.5 py-1.5 ui-text-button text-surface-secondary hover:bg-white transition-colors shrink-0"
              whileTap={{ scale: 0.97 }}
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
              exit={{ opacity: 0 }}
    ...
    return (
      <div className="flex items-center gap-2 rounded-lg border border-red-500/20 bg-red-500/5 px-3 py-2 h-[52px]">
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="ui-text-meta ui-color-error-subtle truncate" title={error}>
            {error}
          </p>
        </div>
        <motion.button
