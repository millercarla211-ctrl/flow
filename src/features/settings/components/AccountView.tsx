...
const getOsIcon = (osName: string, clientName: string) => {
  ...
  if (lowerOs.includes("mac") || lowerOs.includes("darwin") || lowerOs.includes("ios"))
    return <AppleIcon className="text-content-secondary w-4 h-4" />;
  ...
  if (lowerOs.includes("linux") || lowerOs.includes("ubuntu") || lowerOs.includes("debian"))
    return <LinuxIcon className="text-content-secondary w-4 h-4" />;
...
import { getCloudUsageStats, getCachedUsageStats, type CloudUsageStats } from "../../auth/api";
        ...
        <div className="flex items-center gap-4">
          <div className="relative">
            ...
            {isSubscriber && (
              <div className="absolute -bottom-1 -right-1 h-6 w-6 rounded-full bg-surface-primary flex items-center justify-center p-0.5">
                <div className="h-full w-full rounded-full bg-content-primary flex items-center justify-center text-surface-secondary">
          ...
          <div className="group">
              ...
              {isEditingName ? (
                <div className="flex items-center gap-2 h-[28px]">
                  <input
                    ...
                    className="bg-surface-surface border border-border-primary rounded-lg px-2 py-0 ui-text-title-lg font-medium ui-color-on-solid focus:border-border-hover outline-hidden w-48 h-full"
                    onKeyDown={(e) => {
            ...
            >
              ...
              <Pencil
                size={10}
                className="opacity-0 group-hover/pass:opacity-100 transition-opacity"
                aria-hidden="true"
              />
        ...
        <div className="bg-surface-surface border border-border-primary rounded-xl overflow-hidden divide-y divide-surface-elevated">
          ...
          <div className="flex items-center justify-between p-4 transition-colors">
            <div className="flex items-center gap-3">
              <div>
                <div
                  className={`ui-text-body-strong ${isSubscriber ? "ui-color-primary" : "ui-color-muted"}`}
                >
                  {t({
      ...
      <div className="space-y-3">
        <div className="flex items-center justify-between">
          ...
          <button
            ...
            className={`ui-text-meta transition-colors flex items-center justify-start gap-1.5 w-[72px] mr-2 ${
              usageStatsLoading
                ? "ui-color-primary"
          ...
          >
            <RefreshCw
              size={10}
              className={`flex-shrink-0 ${usageStatsLoading ? "animate-spin" : ""}`}
            />
            {usageStatsLoading
              ? t({
                    ...
                    <div className="text-right">
                      <div className="ui-text-kbd ui-color-secondary leading-none mb-1">
                        <span className="text-content-primary">
                          {usageStats.cloud_minutes_this_month.toFixed(0)}
                        </span>
        ...
        <div className="bg-surface-surface border border-border-primary rounded-xl overflow-hidden divide-y divide-surface-elevated">
          {sessionsLoading ? (
            ...
            sessions.map((session, index) => {
              ...
              const sessionKey =
                session.$id ||
                `session-${index}-${session.clientName || session.osName || "unknown"}`;

              return (
                <div
                  key={sessionKey}
                  className="flex items-center justify-between p-4 hover:bg-surface-elevated transition-colors group"
                >
                  <div className="flex items-center gap-3">
                    ...
                    <div className="flex flex-col">
                      <div className="flex items-center gap-2">
                        ...
                        {session.current && (
                          <span className="ui-text-micro font-semibold ui-color-primary bg-surface-elevated px-1.5 py-0.5 rounded-sm">
                            {t({
              ...
              {passwordSuccess ? (
                ...
                <form onSubmit={handlePasswordChange} className="space-y-4">
                  ...
                  <button
                    ...
                    className="w-full bg-content-primary hover:bg-white text-surface-secondary font-medium rounded-xl py-2.5 ui-text-body transition-colors disabled:opacity-50 mt-2"
                  >
                    {passwordLoading ? (
