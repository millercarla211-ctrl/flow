...
pub enum Command {
    ...
    /// Write a Friday release handoff packet
    FridayReleaseHandoffPacket {
        packet_file: String,
        attachment_review_file: String,
    },
    /// Print a Friday release handoff packet as JSON
    FridayReleaseHandoffPacketJson {
        packet_file: String,
        attachment_review_file: String,
    },
    ...
    /// Write a Friday release handoff dispatch governance review
    FridayReleaseHandoffDispatchGovernance {
        review_file: String,
        trail_file: String,
    },
    /// Print a Friday release handoff dispatch governance review as JSON
    FridayReleaseHandoffDispatchGovernanceJson {
        review_file: String,
        trail_file: String,
    },
...
impl Args {
    ...
    pub fn parse() -> Self {
        ...
        let command = match args[1].as_str() {
            ...
            "--friday-release-handoff-packet" | "--friday-handoff-packet" => {
                let (packet_file, attachment_review_file) =
                    parse_friday_release_handoff_packet_args(&args);
                Command::FridayReleaseHandoffPacket {
                    packet_file,
                    attachment_review_file,
                }
            }
            "--friday-release-handoff-packet-json" | "--friday-handoff-packet-json" => {
                let (packet_file, attachment_review_file) =
                    parse_friday_release_handoff_packet_args(&args);
            ...
            | "--friday-handoff-dispatch-governance" => {
                let (review_file, trail_file) =
                    parse_friday_release_handoff_dispatch_governance_args(&args);
                Command::FridayReleaseHandoffDispatchGovernance {
                    review_file,
                    trail_file,
                }
            }
            "--friday-release-handoff-dispatch-governance-json"
            | "--friday-handoff-dispatch-governance-json" => {
                let (review_file, trail_file) =
                    parse_friday_release_handoff_dispatch_governance_args(&args);
...
fn parse_friday_release_handoff_packet_args(args: &[String]) -> (String, String) {
    let export_dir = flag_value(args, "--export-dir").unwrap_or_else(|| {
        args.get(2)
            .filter(|value| !value.starts_with("--"))
            .cloned()
    ...
    let packet_file = flag_value(args, "--output")
        .or_else(|| flag_value(args, "--packet"))
        .unwrap_or_else(|| format!("{export_dir}/release-handoff-packet.json"));
    let attachment_review_file = flag_value(args, "--attachment-review")
        .or_else(|| flag_value(args, "--review"))
        .or_else(|| flag_value(args, "--input"))
        .unwrap_or_else(|| format!("{export_dir}/release-evidence-attachment-review.json"));
...
fn parse_friday_release_handoff_dispatch_governance_args(args: &[String]) -> (String, String) {
    let export_dir = flag_value(args, "--export-dir").unwrap_or_else(|| {
        args.get(2)
            .filter(|value| !value.starts_with("--"))
            .cloned()
