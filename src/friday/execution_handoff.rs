...
fn handoff_specs() -> Vec<HandoffSpec> {
    vec![
        ...
        spec(
            ...
            "flow-dictate --file <wav-path> --model parakeet-tdt-0.6b-v3-int8",
            "src/bin/flow-dictate.rs",
            true,
            &["recorded-audio-file", "local-stt", "stdout-transcript"],
            None,
            "flow --friday-local-checks",
            "Use file-mode STT stdout for captured audio; do not advertise a transcript artifact until the command writes one.",
        ),
        spec(
