...
fn friday_dashboard_trusted_host_runner_executes_only_approved_bounded_commands() {
    ...
    let checkpoint_evidence_vault_path = root.join("release-checkpoint-evidence-vault.json");
    let checkpoint_evidence_vault = friday_release_checkpoint_evidence_vault_report(
    ...
    write_friday_release_checkpoint_evidence_vault(
        &checkpoint_evidence_vault_path,
        &checkpoint_evidence_vault,
    )
    .unwrap();
    assert!(!checkpoint_evidence_vault.ready_to_archive);
    assert_eq!(
        checkpoint_evidence_vault.review_id.as_deref(),
        Some(checkpoint_review.review_id.as_str())
    );
    assert_eq!(
        checkpoint_evidence_vault.active_decision,
        Some(FridayReleaseCheckpointSignoffDecision::Held)
    );
    assert!(checkpoint_evidence_vault.entry_count >= 4);
    assert!(checkpoint_evidence_vault.required_count >= 3);
    assert!(checkpoint_evidence_vault.missing_count > 0);
    assert!(checkpoint_evidence_vault.checksum_count > 0);
    assert!(checkpoint_evidence_vault.release_gate_blocking_count > 0);
    assert!(checkpoint_evidence_vault.entries.iter().any(|entry| {
        entry.kind.label() == "acknowledgement-evidence" && entry.required && !entry.present
    }));
    assert!(
        checkpoint_evidence_vault
            .attachment_notes_copy
            .contains("Friday checkpoint evidence vault")
    );
    ...
    let release_handoff_dispatch_governance_review_path =
        root.join("release-handoff-dispatch-governance-review.json");
    let release_handoff_dispatch_governance_review =
        friday_release_handoff_dispatch_governance_review_report(
    ...
    write_friday_release_handoff_dispatch_governance_review(
        &release_handoff_dispatch_governance_review_path,
        &release_handoff_dispatch_governance_review,
    )
    .unwrap();
    assert!(!release_handoff_dispatch_governance_review.approved_for_external_handoff);
    assert_eq!(
        release_handoff_dispatch_governance_review.state,
    ...
    assert_eq!(
        release_handoff_dispatch_governance_review.latest_state,
        Some(FridayReleaseHandoffDispatchAuditState::Blocked)
    );
    assert!(release_handoff_dispatch_governance_review.score_out_of_100 < 100);
    assert!(release_handoff_dispatch_governance_review.finding_count > 0);
    assert!(release_handoff_dispatch_governance_review.blocked_carryover_count > 0);
    assert!(release_handoff_dispatch_governance_review.release_gate_blocking_count > 0);
    assert!(
        release_handoff_dispatch_governance_review
            .findings
            .iter()
            .any(|finding| {
                finding.state == FridayReleaseHandoffDispatchGovernanceState::BlockedCarryover
                    && finding.release_gate_blocking
                    && finding.next_action.contains("external handoff complete")
            })
    );
    assert!(
        release_handoff_dispatch_governance_review
            .governance_notes_copy
            .contains("Friday release handoff dispatch governance")
    );
    assert!(
        release_handoff_dispatch_governance_review
            .commands
    ...
    let release_external_receipt_archive_path = root.join("release-external-receipt-archive.json");
    let release_external_receipt_record =
        friday_release_external_receipt_record_from_outbound_review(
            &release_outbound_review_ledger_path,
            FridayReleaseExternalReceiptRequest {
                state: FridayReleaseExternalReceiptState::Verified,
                receipt_kind: FridayReleaseExternalReceiptKind::Publication,
                operator: "release-operator".to_string(),
                receipt_note: "Receipt evidence stays blocked until outbound review copy is safe."
                    .to_string(),
                evidence_path: Some("tmp/friday-dashboard/manual-receipt.md".to_string()),
        ...
        .unwrap();
    assert_eq!(
        release_external_receipt_record.state,
        FridayReleaseExternalReceiptState::Blocked
    );
    assert!(!release_external_receipt_record.receipt_attached);
    assert!(!release_external_receipt_record.receipt_verified);
    assert!(!release_external_receipt_record.externally_mutated_by_friday);
    assert_eq!(
        release_external_receipt_record.outbound_review_state,
        FridayReleaseOutboundReviewState::Blocked
    );
    assert!(!release_external_receipt_record.outbound_review_copy_safe);
    assert!(
        release_external_receipt_record
            .receipt_notes_copy
            .contains("Friday release external receipt archive")
    );
    assert!(
        release_external_receipt_record
            .receipt_notes_copy
    ...
    let release_external_receipt_archive = friday_release_external_receipt_archive_report(
        &release_external_receipt_archive_path,
        vec![release_external_receipt_record],
    );
    write_friday_release_external_receipt_archive(
        &release_external_receipt_archive_path,
        &release_external_receipt_archive,
    )
    .unwrap();
    assert_eq!(release_external_receipt_archive.record_count, 1);
    assert_eq!(release_external_receipt_archive.blocked_count, 1);
    assert_eq!(release_external_receipt_archive.verified_count, 0);
    assert_eq!(release_external_receipt_archive.attached_receipt_count, 0);
    assert_eq!(release_external_receipt_archive.verified_receipt_count, 0);
    assert_eq!(release_external_receipt_archive.blocked_receipt_count, 1);
    assert!(
        release_external_receipt_archive
            .audit_notes_copy
            .contains("Friday release external receipt archive")
    ...
    assert!(
        release_external_receipt_archive
            ...
            .iter()
            .any(|command| {
                command.contains("--friday-release-external-receipt")
