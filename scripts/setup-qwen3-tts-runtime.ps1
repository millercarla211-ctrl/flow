param(
  [string]$RuntimeVenv = "G:\Flow\runtime\qwen-tts\.venv"
)
...
if ($LASTEXITCODE -ne 0) {
  throw "Failed to install qwen-tts runtime"
}

Write-Host "[qwen3-tts] Runtime ready: $python"
Write-Host "[qwen3-tts] Flow auto-detects this runtime. Override with FLOW_TTS_PYTHON if needed."
